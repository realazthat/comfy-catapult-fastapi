...
if [[ -z "$IMAGE_PREFIX" ]]; then
  ...
  echo -e "${RED}Just set this to any convenient name representing this project, e.g myproject_test_image${NC}"
...
if [[ -z "$INSTANCE" ]]; then
  ...
  echo -e "${RED}Just set this to any convenient name representing this project, e.g myproject_test_instance${NC}"
  [[ $0 == "$BASH_SOURCE" ]] && EXIT=exit || EXIT=return
  $EXIT 1
fi
...
if [[ -z "$ENV_VARS_FILE" ]]; then
  ...
  echo -e "${RED}A bash script with environement vars exported to be passed into the docker instance, must have COMFY_API_URL set.${NC}"
  [[ $0 == "$BASH_SOURCE" ]] && EXIT=exit || EXIT=return
  $EXIT 1
fi
